"""
Phase 3I – Script 1
Creates apps/web/app/api/genlayer/sync-game/route.ts and
         apps/web/app/api/genlayer/record-tx/route.ts

sync-game: Called after a game ends on GenLayer. Fetches the final game state
           from the contract and persists it to Supabase (games status, moves,
           board_cells) and updates the leaderboard.

record-tx: Records a GenLayer transaction hash to the transactions table for
...
# --- sync-game ---
SYNC_TARGET = ROOT / "apps" / "web" / "app" / "api" / "genlayer" / "sync-game" / "route.ts"
...
print(f"[OK] Created {SYNC_TARGET}")

# --- record-tx ---
TX_TARGET = ROOT / "apps" / "web" / "app" / "api" / "genlayer" / "record-tx" / "route.ts"
TX_TARGET.parent.mkdir(parents=True, exist_ok=True)
...
TX_TARGET.write_text(TX_CONTENT, encoding="utf-8")
print(f"[OK] Created {TX_TARGET}")
